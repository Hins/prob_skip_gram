...
class PSGModel():
    def __init__(self, sess, output_file):
        ...
        self.dictionary = tf.placeholder(shape=[cfg.batch_size, cfg.dict_length], dtype=tf.int32)
        ...
        with tf.device('/gpu:0'):
            with tf.variable_scope("psg_model"):
                ...
                self.dictionary_desc_weight = tf.get_variable(
                    'position_emb',
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        PSGModelObj = PSGModel(sess, sys.argv[4])
        ...
        for epoch_index in range(cfg.epoch_size):
            ...
            for i in range(train_set_size):
                ...
                trainable = True
                _, iter_loss = PSGModelObj.train(word1_list[i * cfg.batch_size:(i+1) * cfg.batch_size],
            ...
            train_loss = PSGModelObj.get_loss_summary(np.sum(loss_sum) / cfg.batch_size)
            train_writer.add_summary(train_loss, epoch_index + 1)

            accuracy = 0.0
            for j in range(total_batch_size - train_set_size):
                j += train_set_size
                iter_accuracy, index_score = PSGModelObj.validate(word1_list[j*cfg.batch_size : (j+1)*cfg.batch_size],
            ...
            test_accuracy = PSGModelObj.get_accuracy_summary(accuracy / (total_batch_size - train_set_size))
